...
const CartItems = () => {
  ...
  const [checkout, setCheckout] = useState(
    localStorage.getItem("checkout") === "true" || userCheckout.checkout
  );
    ...
    const makeRequest = async () => {
      try {
        const paymentRes = await axios.post(
          "https://ecommercebackend-11d1.onrender.com/payment",
          {
        ...
        if (paymentRes.data.success) {
          ...
          const saveOrder = await axios.post(
            "https://ecommercebackend-11d1.onrender.com/addToOrder",
            { email: userEmail, orderList: orderProduct }
          );
  ...
  }, [stripeToken, getTotalCartAmount, userEmail, orderProduct, removeAllCart]);
    ...
    <div className="cartitems">
      ...
      <div className="cartitems-down">
        <div className="cartitems-total">
          ...
          0 ? (
            <button disabled style={{ backgroundColor: "lightgray" }}>
              PROCEED TO CHECKOUT
            </button>
          ) : (
            <StripeCheckout
